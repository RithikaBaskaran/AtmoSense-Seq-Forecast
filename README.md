...
The project was developed using Google Colab and UMD Zaratan HPC. The evaluation notebook assumes a Colab-style Google Drive path.

---

## Data and Checkpoints

This project uses hourly air quality monitoring data from India’s Central Pollution Control Board (CPCB), sourced through the Kaggle dataset **Time Series Air Quality Data of India (2010–2023)**. The raw dataset contains station-level CSV files with hourly pollutant readings collected across India.

The final model uses 11 pollutant columns:
...
<p align="center">
  <img src="results/figures/figures/pollutant_correlation_heatmap.png" alt="Pollutant Correlation Heatmap" width="400">
</p>

The preprocessing pipeline filters sparse stations, performs per-station interpolation, applies chronological train/validation/test splitting, normalizes pollutant values using a saved `StandardScaler`, and builds 72-hour input / 48-hour forecast sliding windows.

Raw data, trained model checkpoints, scalers, and cached full-test prediction arrays are not stored directly in this GitHub repository because of file size. Instead, they are provided through a shared Google Drive folder.

For detailed data setup instructions, see:
